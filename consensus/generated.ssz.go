...
// Hash: 69c9cf10e8b75d8e573cecfdbd4f8bff43afb3584a161845c336d3da427c328b
package pbc
...
func (s *StorageContract) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (10) 'PorPenalty'
	dst = ssz.MarshalUint64(dst, s.PorPenalty)

	// Field (11) 'Status'
	dst = ssz.MarshalUint32(dst, s.Status)

	// Field (12) 'StatusStart'
...
func (s *StorageContract) UnmarshalSSZ(buf []byte) error {
	...
	// Field (10) 'PorPenalty'
	s.PorPenalty = ssz.UnmarshallUint64(buf[232:240])

	// Field (11) 'Status'
	s.Status = ssz.UnmarshallUint32(buf[240:244])

	// Field (12) 'StatusStart'
	s.StatusStart = Slot(ssz.UnmarshallUint64(buf[244:252]))

	// Field (13) 'StatusEnd'
	s.StatusEnd = Slot(ssz.UnmarshallUint64(buf[252:260]))

	// Field (14) 'Auditor'
	if cap(s.Auditor) == 0 {
		s.Auditor = make([]byte, 0, len(buf[260:308]))
	}
	s.Auditor = append(s.Auditor, buf[260:308]...)

	// Field (15) 'AuditHash'
	if cap(s.AuditHash) == 0 {
		s.AuditHash = make([]byte, 0, len(buf[308:340]))
	}
	s.AuditHash = append(s.AuditHash, buf[308:340]...)

	// Field (16) 'PorChallengeHash'
	if cap(s.PorChallengeHash) == 0 {
		s.PorChallengeHash = make([]byte, 0, len(buf[340:372]))
	}
	s.PorChallengeHash = append(s.PorChallengeHash, buf[340:372]...)
