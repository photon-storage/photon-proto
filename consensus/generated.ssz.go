...
// Hash: 355aaebdfd73fd0a15cffeaed12da442685e3b7cf665e1371a6d5d0ad580f026
package pbc
...
func (b *BlockBody) UnmarshalSSZ(buf []byte) error {
	...
	{
		buf = tail[o2:o3]
		num, err := ssz.DivideInt2(len(buf), 2480, 1024)
		if err != nil {
			return err
		}
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = b.Txs[ii].UnmarshalSSZ(buf[ii*2480 : (ii+1)*2480]); err != nil {
				return err
			}
		}
	}
...
func (s *StorageContract) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (10) 'Mutable'
	if s.Mutable != nil {
		if dst, err = s.Mutable.MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
func (s *StorageContract) UnmarshalSSZ(buf []byte) error {
	...
	// Field (10) 'Mutable'
	if s.Mutable == nil {
		s.Mutable = new(StorageContract_Mutable)
	}
	if err = s.Mutable.UnmarshalSSZ(buf[232:372]); err != nil {
		return err
	}
...
func (s *StorageContract) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	// Field (10) 'Mutable'
	if s.Mutable != nil {
		if err = s.Mutable.HashTreeRootWith(hh); err != nil {
			return
		}
	}
...
func (a *Audit) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (1) 'Start'
	dst = ssz.MarshalUint64(dst, uint64(a.Start))
...
func (a *Audit) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'Start'
	a.Start = Slot(ssz.UnmarshallUint64(buf[48:56]))
...
func (b *BlockPick) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (0) 'BlockIndex'
	dst = ssz.MarshalUint32(dst, b.BlockIndex)

	// Field (1) 'Coefficient'
	if len(b.Coefficient) != 32 {
		err = ssz.ErrBytesLength
		return
	}
	dst = append(dst, b.Coefficient...)
...
func (b *BlockPick) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'BlockIndex'
	b.BlockIndex = ssz.UnmarshallUint32(buf[0:4])

	// Field (1) 'Coefficient'
	if cap(b.Coefficient) == 0 {
		b.Coefficient = make([]byte, 0, len(buf[4:36]))
	}
	b.Coefficient = append(b.Coefficient, buf[4:36]...)
...
func (p *PoRChallenge) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (0) 'Challenger'
	if len(p.Challenger) != 48 {
		err = ssz.ErrBytesLength
		return
	}
	dst = append(dst, p.Challenger...)

	// Field (1) 'Start'
	dst = ssz.MarshalUint64(dst, uint64(p.Start))

	// Field (2) 'End'
	dst = ssz.MarshalUint64(dst, uint64(p.End))
	...
	for ii := 0; ii < len(p.Picks); ii++ {
		if dst, err = p.Picks[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
func (p *PoRChallenge) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'Challenger'
	if cap(p.Challenger) == 0 {
		p.Challenger = make([]byte, 0, len(buf[0:48]))
	}
	p.Challenger = append(p.Challenger, buf[0:48]...)

	// Field (1) 'Start'
	p.Start = Slot(ssz.UnmarshallUint64(buf[48:56]))

	// Field (2) 'End'
	p.End = Slot(ssz.UnmarshallUint64(buf[56:64]))

	// Field (3) 'ObjectHash'
	if cap(p.ObjectHash) == 0 {
		p.ObjectHash = make([]byte, 0, len(buf[64:96]))
	}
	p.ObjectHash = append(p.ObjectHash, buf[64:96]...)

	// Field (4) 'AuditHash'
	if cap(p.AuditHash) == 0 {
		p.AuditHash = make([]byte, 0, len(buf[96:128]))
	}
	p.AuditHash = append(p.AuditHash, buf[96:128]...)

	// Field (5) 'Auditor'
	if cap(p.Auditor) == 0 {
		p.Auditor = make([]byte, 0, len(buf[128:176]))
	}
	p.Auditor = append(p.Auditor, buf[128:176]...)

	// Offset (6) 'Picks'
	if o6 = ssz.ReadOffset(buf[176:180]); o6 > size {
		return ssz.ErrOffset
	}
	...
	// Field (6) 'Picks'
	{
		...
		p.Picks = make([]*BlockPick, num)
		for ii := 0; ii < num; ii++ {
			if p.Picks[ii] == nil {
				p.Picks[ii] = new(BlockPick)
			}
			if err = p.Picks[ii].UnmarshalSSZ(buf[ii*36 : (ii+1)*36]); err != nil {
				return err
			}
		}
	}
...
func (p *PoRChallenge) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	hh.PutBytes(p.Challenger)

	// Field (1) 'Start'
	hh.PutUint64(uint64(p.Start))

	// Field (2) 'End'
	hh.PutUint64(uint64(p.End))
	...
	{
		subIndx := hh.Index()
		num := uint64(len(p.Picks))
		if num > 32 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range p.Picks {
			if err = elem.HashTreeRootWith(hh); err != nil {
				return
			}
		}
...
func (s *StorageContract_Mutable) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (1) 'StatusStart'
	dst = ssz.MarshalUint64(dst, uint64(s.StatusStart))

	// Field (2) 'StatusEnd'
	dst = ssz.MarshalUint64(dst, uint64(s.StatusEnd))
	...
	// Field (5) 'PorChallengeHash'
	if len(s.PorChallengeHash) != 32 {
		err = ssz.ErrBytesLength
		return
	}
	dst = append(dst, s.PorChallengeHash...)

	// Field (6) 'PledgeRemaining'
	dst = ssz.MarshalUint64(dst, s.PledgeRemaining)
...
func (s *StorageContract_Mutable) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'Status'
	s.Status = ssz.UnmarshallUint32(buf[0:4])

	// Field (1) 'StatusStart'
	s.StatusStart = Slot(ssz.UnmarshallUint64(buf[4:12]))

	// Field (2) 'StatusEnd'
	s.StatusEnd = Slot(ssz.UnmarshallUint64(buf[12:20]))

	// Field (3) 'Auditor'
	if cap(s.Auditor) == 0 {
		s.Auditor = make([]byte, 0, len(buf[20:68]))
	}
	s.Auditor = append(s.Auditor, buf[20:68]...)

	// Field (4) 'AuditHash'
	if cap(s.AuditHash) == 0 {
		s.AuditHash = make([]byte, 0, len(buf[68:100]))
	}
	s.AuditHash = append(s.AuditHash, buf[68:100]...)

	// Field (5) 'PorChallengeHash'
	if cap(s.PorChallengeHash) == 0 {
		s.PorChallengeHash = make([]byte, 0, len(buf[100:132]))
	}
	s.PorChallengeHash = append(s.PorChallengeHash, buf[100:132]...)

	// Field (6) 'PledgeRemaining'
	s.PledgeRemaining = ssz.UnmarshallUint64(buf[132:140])
...
func (s *StorageContract_Mutable) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	// Field (1) 'StatusStart'
	hh.PutUint64(uint64(s.StatusStart))

	// Field (2) 'StatusEnd'
	hh.PutUint64(uint64(s.StatusEnd))
	...
	hh.PutBytes(s.PorChallengeHash)

	// Field (6) 'PledgeRemaining'
	hh.PutUint64(s.PledgeRemaining)
...
func (s *State) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (8) 'InactivityScores'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(s.InactivityScores) * 8

	// Offset (9) 'PreviousEpochParticipation'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(s.PreviousEpochParticipation)

	// Offset (10) 'CurrentEpochParticipation'
	dst = ssz.WriteOffset(dst, offset)
	...
	// Field (16) 'RandaoMixesTrieRoot'
	if len(s.RandaoMixesTrieRoot) != 32 {
		err = ssz.ErrBytesLength
		return
	}
	dst = append(dst, s.RandaoMixesTrieRoot...)
	...
	// Field (19) 'SlotTrieRoot'
	if len(s.SlotTrieRoot) != 32 {
		err = ssz.ErrBytesLength
		return
	}
	dst = append(dst, s.SlotTrieRoot...)

	// Field (20) 'ReserviorBalance'
	...
	// Field (9) 'PreviousEpochParticipation'
	...
	// Field (10) 'CurrentEpochParticipation'
...
func (s *State) UnmarshalSSZ(buf []byte) error {
	...
	if cap(s.AccountTrieRoot) == 0 {
		s.AccountTrieRoot = make([]byte, 0, len(buf[297:329]))
	}
	s.AccountTrieRoot = append(s.AccountTrieRoot, buf[297:329]...)

	// Field (13) 'ValidatorTrieRoot'
	if cap(s.ValidatorTrieRoot) == 0 {
		s.ValidatorTrieRoot = make([]byte, 0, len(buf[329:361]))
	}
	s.ValidatorTrieRoot = append(s.ValidatorTrieRoot, buf[329:361]...)

	// Field (14) 'AuditorTrieRoot'
	if cap(s.AuditorTrieRoot) == 0 {
		s.AuditorTrieRoot = make([]byte, 0, len(buf[361:393]))
	}
	s.AuditorTrieRoot = append(s.AuditorTrieRoot, buf[361:393]...)

	// Field (15) 'StorageTrieRoot'
	if cap(s.StorageTrieRoot) == 0 {
		s.StorageTrieRoot = make([]byte, 0, len(buf[393:425]))
	}
	s.StorageTrieRoot = append(s.StorageTrieRoot, buf[393:425]...)

	// Field (16) 'RandaoMixesTrieRoot'
	if cap(s.RandaoMixesTrieRoot) == 0 {
		s.RandaoMixesTrieRoot = make([]byte, 0, len(buf[425:457]))
	}
	s.RandaoMixesTrieRoot = append(s.RandaoMixesTrieRoot, buf[425:457]...)

	// Field (17) 'AuditQueueRoot'
	if cap(s.AuditQueueRoot) == 0 {
		s.AuditQueueRoot = make([]byte, 0, len(buf[457:489]))
	}
	s.AuditQueueRoot = append(s.AuditQueueRoot, buf[457:489]...)

	// Field (18) 'EventQueueRoot'
	if cap(s.EventQueueRoot) == 0 {
		s.EventQueueRoot = make([]byte, 0, len(buf[489:521]))
	}
	s.EventQueueRoot = append(s.EventQueueRoot, buf[489:521]...)

	// Field (19) 'SlotTrieRoot'
	if cap(s.SlotTrieRoot) == 0 {
		s.SlotTrieRoot = make([]byte, 0, len(buf[521:553]))
	}
	s.SlotTrieRoot = append(s.SlotTrieRoot, buf[521:553]...)

	// Field (20) 'ReserviorBalance'
	s.ReserviorBalance = ssz.UnmarshallUint64(buf[553:561])

	// Offset (21) 'Storage'
	if o21 = ssz.ReadOffset(buf[561:565]); o21 > size || o11 > o21 {
		return ssz.ErrOffset
	}
...
func (s *State) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	hh.PutBytes(s.RandaoMixesTrieRoot)
...
func (s *Storage) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (3) 'FeeRate'
	dst = ssz.MarshalUint64(dst, s.FeeRate)

	// Field (4) 'FeeRateSince'
	dst = ssz.MarshalUint64(dst, uint64(s.FeeRateSince))
...
func (s *Storage) UnmarshalSSZ(buf []byte) error {
	...
	// Field (3) 'FeeRate'
	s.FeeRate = ssz.UnmarshallUint64(buf[20:28])

	// Field (4) 'FeeRateSince'
	s.FeeRateSince = Slot(ssz.UnmarshallUint64(buf[28:36]))
...
func (s *Storage) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	// Field (3) 'FeeRate'
	hh.PutUint64(s.FeeRate)

	// Field (4) 'FeeRateSince'
	hh.PutUint64(uint64(s.FeeRateSince))
...
func (t *Transaction) UnmarshalSSZ(buf []byte) error {
	...
	if err = t.TxDataObjectAudit.UnmarshalSSZ(buf[908:1920]); err != nil {
		return err
	}
	...
	if err = t.TxDataObjectChallenge.UnmarshalSSZ(buf[1920:2000]); err != nil {
		return err
	}
	...
	if err = t.TxDataObjectPoR.UnmarshalSSZ(buf[2000:2384]); err != nil {
		return err
	}
...
func (t *TxDataObjectAudit) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (1) 'AuditHash'
	if len(t.AuditHash) != 32 {
		err = ssz.ErrBytesLength
		return
	}
	dst = append(dst, t.AuditHash...)
...
func (t *TxDataObjectAudit) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'AuditHash'
	if cap(t.AuditHash) == 0 {
		t.AuditHash = make([]byte, 0, len(buf[32:64]))
	}
	t.AuditHash = append(t.AuditHash, buf[32:64]...)

	// Field (2) 'Auditor'
	if cap(t.Auditor) == 0 {
		t.Auditor = make([]byte, 0, len(buf[64:112]))
	}
	t.Auditor = append(t.Auditor, buf[64:112]...)

	// Field (3) 'Depot'
	if cap(t.Depot) == 0 {
		t.Depot = make([]byte, 0, len(buf[112:160]))
	}
	t.Depot = append(t.Depot, buf[112:160]...)

	// Field (4) 'Hash'
	if cap(t.Hash) == 0 {
		t.Hash = make([]byte, 0, len(buf[160:192]))
	}
	t.Hash = append(t.Hash, buf[160:192]...)

	// Field (5) 'Size'
	t.Size = ssz.UnmarshallUint64(buf[192:200])

	// Field (6) 'EncodedHash'
	if cap(t.EncodedHash) == 0 {
		t.EncodedHash = make([]byte, 0, len(buf[200:232]))
	}
	t.EncodedHash = append(t.EncodedHash, buf[200:232]...)

	// Field (7) 'EncodedSize'
	t.EncodedSize = ssz.UnmarshallUint64(buf[232:240])

	// Field (8) 'NumBlocks'
	t.NumBlocks = ssz.UnmarshallUint32(buf[240:244])

	// Field (9) 'Rands'
	t.Rands = make([][]byte, 8)
	for ii := 0; ii < 8; ii++ {
		if cap(t.Rands[ii]) == 0 {
			t.Rands[ii] = make([]byte, 0, len(buf[244:1012][ii*96:(ii+1)*96]))
		}
		t.Rands[ii] = append(t.Rands[ii], buf[244:1012][ii*96:(ii+1)*96]...)
	}
...
func (s *SignedTransaction) UnmarshalSSZ(buf []byte) error {
	...
	if err = s.Tx.UnmarshalSSZ(buf[0:2384]); err != nil {
		return err
	}

	// Field (1) 'Signature'
	if cap(s.Signature) == 0 {
		s.Signature = make([]byte, 0, len(buf[2384:2480]))
	}
	s.Signature = append(s.Signature, buf[2384:2480]...)
...
func (v *Validator) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'Balance'
	v.Balance = ssz.UnmarshallUint64(buf[48:56])

	// Field (2) 'Slashed'
	v.Slashed = ssz.UnmarshalBool(buf[56:57])

	// Field (3) 'ActivationEligibilityEpoch'
	v.ActivationEligibilityEpoch = Epoch(ssz.UnmarshallUint64(buf[57:65]))

	// Field (4) 'ActivationEpoch'
	v.ActivationEpoch = Epoch(ssz.UnmarshallUint64(buf[65:73]))

	// Field (5) 'ExitEpoch'
	v.ExitEpoch = Epoch(ssz.UnmarshallUint64(buf[73:81]))

	// Field (6) 'WithdrawableEpoch'
	v.WithdrawableEpoch = Epoch(ssz.UnmarshallUint64(buf[81:89]))
