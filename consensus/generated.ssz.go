...
// Hash: 5cc64d965e4dece09ebb7cef982dd922e13842253bd53846a194e6b9e54db4fc
package pbc
...
func (s *StorageContract) UnmarshalSSZ(buf []byte) error {
	...
	// Field (11) 'StatusStart'
	s.StatusStart = Slot(ssz.UnmarshallUint64(buf[236:244]))

	// Field (12) 'StatusEnd'
	s.StatusEnd = Slot(ssz.UnmarshallUint64(buf[244:252]))

	// Field (13) 'Auditor'
	if cap(s.Auditor) == 0 {
		s.Auditor = make([]byte, 0, len(buf[252:300]))
	}
	s.Auditor = append(s.Auditor, buf[252:300]...)

	// Field (14) 'AuditHash'
	if cap(s.AuditHash) == 0 {
		s.AuditHash = make([]byte, 0, len(buf[300:332]))
	}
	s.AuditHash = append(s.AuditHash, buf[300:332]...)

	// Field (15) 'PorChallengeHash'
	if cap(s.PorChallengeHash) == 0 {
		s.PorChallengeHash = make([]byte, 0, len(buf[332:364]))
	}
	s.PorChallengeHash = append(s.PorChallengeHash, buf[332:364]...)
...
func (p *PoRChallenge) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'End'
	p.End = Slot(ssz.UnmarshallUint64(buf[8:16]))

	// Field (2) 'ObjectHash'
	if cap(p.ObjectHash) == 0 {
		p.ObjectHash = make([]byte, 0, len(buf[16:48]))
	}
	p.ObjectHash = append(p.ObjectHash, buf[16:48]...)

	// Field (3) 'AuditHash'
	if cap(p.AuditHash) == 0 {
		p.AuditHash = make([]byte, 0, len(buf[48:80]))
	}
	p.AuditHash = append(p.AuditHash, buf[48:80]...)

	// Field (4) 'Auditor'
	if cap(p.Auditor) == 0 {
		p.Auditor = make([]byte, 0, len(buf[80:128]))
	}
	p.Auditor = append(p.Auditor, buf[80:128]...)

	// Offset (5) 'Picks'
	if o5 = ssz.ReadOffset(buf[128:132]); o5 > size {
		return ssz.ErrOffset
	}
